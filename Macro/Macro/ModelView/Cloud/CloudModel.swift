...
class CloudKitModel {
    ...
    // MARK: fecth
    func fetchSharedPrivatedRecords(recordType: String) async throws -> [CKRecord] {
        ...
        return try await withThrowingTaskGroup(of: [CKRecord].self, returning: [CKRecord].self) { group in
            for zone in sharedZones {
                group.addTask { [self] in
                    do {
                        let fecthShared = try await self.fetchRecords( in: zone.zoneID, from: self.databaseShared, recordType: recordType)
                        let fecthPrivate = try await self.fetchRecords( in: zone.zoneID, from: self.databasePrivate, recordType: recordType)
    ...
    private func fetchRecords(in zone: CKRecordZone.ID? = SharedZone.ZoneID, from database: CKDatabase, recordType: String) async throws -> [CKRecord] {
