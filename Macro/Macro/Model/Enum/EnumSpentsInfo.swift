//
//  EnumSpentsInfo.swift
//  Macro
//
//  Created by Gabriele Namie on 30/09/22.
//

import SwiftUI

enum EnumSpentsinfo: String {
    case essencialsColor = "EssenciaisColor"
    case priorityColor = "PrioridadesColor"
    case leisureColor = "LazerColor"
    case backgroundSpentsColor = "BackgroundSpents-1"
    case secondPriorityColor = "SecondPriorityColor"
    case secondleisureColor = "SecondLeisureColor"
    case secondEssencialsColor = "SecondEssencialsColor"
    case essencialsTitle = "Essenciais"
    case priorityTitle = "Prioridades"
    case leisureTitle = "Lazer"
}
